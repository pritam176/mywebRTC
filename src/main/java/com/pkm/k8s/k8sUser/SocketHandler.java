	...
	protected void handleTextMessage(WebSocketSession session,
		...
		if (LOGIN_TYPE.equalsIgnoreCase(signalMessage.getType())) {
			...
			if (destSocket != null && destSocket.isOpen()) {
				...
				// We write the message to send to the dest socket (it's our
				// propriatary format)

				SignalMessage out = new SignalMessage();
				// still an RTC type
				out.setType(signalMessage.getType());
				// we use the dest field to specify the actual exp., but it will
				// be the next dest.
				...
				String stringifiedJSONmsg = objectMapper
